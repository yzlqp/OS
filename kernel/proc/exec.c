...
/* 
 * Loads the executable image segment 
 */
...
int exec(char *path, char **argv)
{
    ...
    for (argc = 0; argv[argc]; argc ++ ) {
        ...
        // Stack alignment is required under AARCH64 architecture
        sp -= sp % 16; 
        if (sp < stack_base) {
    ...
    // Exec returns the equivalent of calling main for the user process and x0, 
    // argc, returns the value configuration through exec 
    p->tf->regs[1] = sp; 
    // Debugging uses the program name to be copied into proc
    char *last = path;
    for (char *s = path; *s; ++s)
        if (*s == '/') 
            last = s + 1;
    strncpy(p->name, last, sizeof(p->name));
    // Temporarily store the previous page table
